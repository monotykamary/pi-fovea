...
4. **`fovea_impact`** — blast radius. Seed with repo-relative `files`, symbols, uncommitted changes, or a PR `base`. Output is likely review order with causal channels (calls, imports, literals, routes, tests, inheritance, co-change). Structured details add `expectedButUnchanged` (directionally co-coupled files absent from this diff — an omitted-edit alarm), `conservedMass` (graph-size-comparable per-file heat), and `obligations`/`epoch` (a persistent checklist: entries survive disclosure and time until reads/edits/verification resolve them — check it before claiming a feature complete).
