...
4. **`fovea_impact`**: blast radius. Seed with repo-relative `files`, symbols, uncommitted changes, or a PR `base`. Output is heat-ranked likely review order with causal channels (calls, imports, literals, routes, tests, inheritance, co-change). Structured details include per-file `warmedMass`, `warmedReasons`, `warmedEvidence`, `expectedButUnchanged` (likely co-change companions, not required edits), and `conservedMass` (comparable diffusion heat). Results describe the current cascade, not inspection history or unfinished work. Keep tests and acceptance checks in the host workflow.
...
Budget and display-cap overflow are not dead ends: when its tmp artifact is writable, the footer names `/tmp/pi-fovea-<op>-<hash>.txt`, containing every eligible candidate—including the tail beyond 400 display candidates. Read or grep it for the remainder. The list still respects scope, disclosure suppression, and the heat cutoff; it is not every graph node. If no artifact path is present, do not assume a spill succeeded. Use `fovea_dwell` to widen the neighborhood.

`details.coverage.imports` reports possible/unresolved imports, bounded examples, cap counts, and languages without import readers. A **possible import** is an in-scope candidate family, not an exact dependency or complete runtime target set. Use the graph to find source worth inspecting; it does not establish requirements or prove feature completeness.
