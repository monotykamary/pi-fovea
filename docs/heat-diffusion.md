...
Co-change history suggests which files usually move together. A directional residual highlights likely companions absent from the current diff. It complements diffusion heat without establishing mandatory edits:
...
`impact` details surface residuals as `expectedButUnchanged`, strongest first, capped at 12. They are current, history-derived suggestions, not required edits. Residuals cool as history ages. The signal disappears the moment the companion joins the diff.
